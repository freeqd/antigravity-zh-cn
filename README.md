...
- 🌐 **繁體中文／简体中文雙語介面**：涵蓋主介面、設定頁、Agent 管理、MCP／知識庫頁面等多個區域
...
本套件自 v1.0.8 起同時支援**繁體中文（台灣）**與**简体中文（大陆）**兩種漢化語言：

- 執行安裝腳本（Windows：`install-win.bat`；macOS：`install-macos.command`）時，會先詢問要安裝**簡體**或**繁體**，輸入 `1` 安裝簡體中文，輸入 `2`（或直接按 Enter）安裝繁體中文。
- 也可在命令列直接指定語言，例如：
...
- 每次安裝都會以目前語言重新注入，重複安裝即可切換簡體／繁體，無須先還原。
- 兩種語言共用同一套引擎與安裝流程，字典目錄分別為 `dicts/`（繁體）與 `dicts-zh-CN/`（簡體）。
